...
Falls das Smartphone verwendet werden soll, ist es wichtig, dass zuerst die Desktop-Appliaktion gestartet wird und erst danach das Smartphone, da sonst keine Verbindung zwischen den beiden Applikationen hergestellt werden kann.

* Starterklasse starten

image::startDesktopApp.png[]

* Smartphone-App auf einem Android-Device (oder dem Emulator) installieren und starten

image::Smartphone-App.png[]

Um die Smartphone-App starten zu können, muss aktuell noch im Model ( _[silver]#app > src > main > kotlin > ch > model > Model#_ ) in der Variable _mqttBroker_ die IP-Adresse des Desktop-Devices gesetzt werden.

[source, kotlin]
var mqttBroker    = "192.168.0.94" //Beispiel

( _[silver]#app > src > main > kotlin > ch > model > Model#_ )

IP-Adresse herausfinden: +
Terminal öffnen und _ifconfig_ eingeben. Im Output ist unter _en0:_ unter anderem die IP-Adresse zu finden (hinter dem Wort _inet_)
...
=== Smartphone über QR-Code verbinden
